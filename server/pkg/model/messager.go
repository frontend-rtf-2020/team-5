...
type InfoChat struct {
	...
	Onlines         []time.Time `json:"onlines"`
}
...
func GetUsersForChatInfo(mongoClient *mongo.Client, id []string, logins []string,identity *User) InfoChat {
	...
	for i := 0; i < idLength; i++ {
		...
		result = InfoChat{
			LoginList: append(result.LoginList,user.Login ),
			NameList: append(result.NameList,user.Name),
			Onlines: append(result.Onlines, user.Online),
		}
	}
	return result
}
...
func UpdateUserOnline(client *mongo.Client, identity *User){
	usersCollection := client.Database("chat").Collection("users")
	ctx, _ := context.WithTimeout(context.Background(), 15*time.Second)
	id, _ := primitive.ObjectIDFromHex(identity.Id)
	_, _ = usersCollection.UpdateOne(ctx, bson.M{"_id": id}, bson.M{"$set": bson.M{"online": time.Now()}})
}

func ActivateUser(client *mongo.Client, user *User){
	usersCollection := client.Database("chat").Collection("users")
	ctx, _ := context.WithTimeout(context.Background(), 15*time.Second)
	id, _ := primitive.ObjectIDFromHex(user.Id)
	_, _ = usersCollection.UpdateOne(ctx, bson.M{"_id": id}, bson.M{"$set": bson.M{"confirmed": true}})
}