...
func Login(c *gin.Context) {
	...
	if !user.Confirmed{
		respErr(c, nil, "account not confirmed")
		return
	}
...
func Register(c *gin.Context) {
	mongoClient := c.Keys["mongo"].(*mongo.Client)
	mailerClient := c.Keys["mailer"].(*mailer.Mailer)

	confirmationCode := randToken()
	...
	reqPayload.ConfirmationCode = confirmationCode
	reqPayload.Confirmed = false
	...
	mailerClient.SendConfirmationEmail(mailer.EMail{
		To: reqPayload.Email,
...
func ConfirmAccount(c *gin.Context){
	...
	if err != nil {
		respErr(c, err, "failed to decode req body")
		return
	}
	ctx, _ := context.WithTimeout(context.Background(), 15*time.Second)

	usersCollection := mongoClient.Database("chat").Collection("users")
	resp := usersCollection.FindOne(ctx, bson.M{"confirmation_code": reqPayload.ConfirmationCode})
	...
	model.ActivateUser(mongoClient, &user)
